    ...
    <div className="modal modal-open">
      <div className="modal-box max-w-sm rounded-2xl border border-base-200 bg-base-100 p-6 shadow-2xl">
        ...
        <h3 className="text-base font-bold text-center mb-1">
          {language === 'de' ? 'Buchung löschen?' : 'Delete transaction?'}
        </h3>
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            onClick={() => del(delId)}
          >
            {isDeleting ? (language === 'de' ? 'Löschen...' : 'Deleting...') : (language === 'de' ? 'Löschen' : 'Delete')}
          </button>
        </div>
      </div>
      <div className="modal-backdrop" onClick={() => !isDeleting && setDelId(null)} />
    </div>
  );
}
