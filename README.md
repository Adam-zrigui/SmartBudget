...
Set the following in your deployment (Vercel, host, or local .env):

- `DATABASE_URL` — Postgres connection string for Prisma
- `NEXTAUTH_URL` — https://your-domain.example
- `NEXTAUTH_SECRET` — long random secret
- `NEXT_PUBLIC_APP_URL` — public site URL
- Any third-party API keys (Sentry, Stripe, Google OAuth, etc.)

Deploy to Vercel

1. Push the repo to GitHub.
2. In Vercel, create a new project and import the repo. Vercel detects Next.js automatically.
3. Set the env vars in Project Settings → Environment Variables.
4. Vercel will run `pnpm build` on deploy.

Notes

- Favicons and `site.webmanifest` are stored in `public/`.
- If you remove server-side PDF support, also remove the `format === "pdf"` branch in `app/api/transactions/export/route.ts`.# 💰 SmartBudget
