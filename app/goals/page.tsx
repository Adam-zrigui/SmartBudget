...
export default function GoalsPage() {
  ...
  const loadGoals = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setLoadError('Konnte Sparziele nicht laden. Bitte erneut versuchen.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  async function updateGoal(goal: any) {
    try {
  ...
  async function deleteGoal(id: string) {
    try {
      const res = await authedFetch(`/api/savings-goals/${id}`, { method: 'DELETE' });
  ...
  async function addContribution(contribution: any) {
    try {
  ...
  return (
    <div className="flex min-h-screen bg-base-100 text-base-content">
      ...
      <div className="hidden lg:flex lg:shrink-0">
        <Sidebar taxResult={{}} txsLength={0} tab="goals" setTab={handleTabChange} />
      </div>
        ...
        <main className="flex-1 overflow-y-auto p-5 lg:p-8 bg-base-100">
          <div className="max-w-6xl mx-auto">
            <div className="mb-4 flex justify-end">
              <Button variant="outline" onClick={() => loadGoals(true)} disabled={refreshing}>
