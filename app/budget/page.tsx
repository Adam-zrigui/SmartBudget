...
export default function BudgetPage() {
  ...
  const loadPageData = useCallback(async (silent = false) => {
    ...
    try {
      ...
      setLoadError('Konnte Budgetdaten nicht laden. Bitte erneut versuchen.');
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  async function deleteBudget(id: string) {
    try {
      const res = await authedFetch(`/api/budgets/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="flex min-h-screen bg-base-100 text-base-content">
      ...
      <div className="hidden lg:flex lg:shrink-0">
        <Sidebar taxResult={{}} txsLength={0} tab="budget" setTab={handleTabChange} />
      </div>
        ...
        <main className="flex-1 overflow-y-auto p-5 lg:p-8 bg-base-100">
          <div className="max-w-6xl mx-auto">
            <div className="mb-4 flex justify-end">
              <Button variant="outline" onClick={() => loadPageData(true)} disabled={refreshing}>
